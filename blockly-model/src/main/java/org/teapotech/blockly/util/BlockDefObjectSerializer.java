...
import org.teapotech.blockly.block.def.BlockField;
...
public class BlockDefObjectSerializer extends StdSerializer<BlockDefObject> {

	private static final long serialVersionUID = 5059632914672689100L;

	protected BlockDefObjectSerializer() {
		super(BlockDefObject.class);
	}

	@Override
	public void serialize(BlockDefObject value, JsonGenerator gen, SerializerProvider provider) throws IOException {
		gen.writeStartObject();
		gen.writeStringField("type", value.getType());
		StringBuilder messageBuilder = new StringBuilder();
		List<BlockField> args = value.getArgs();
		if (args != null) {
			gen.writeArrayFieldStart("args0");
			int idx = 1;
			for (BlockField bFld : args) {
